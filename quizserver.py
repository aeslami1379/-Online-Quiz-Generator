...
class QuizRequestHandler(BaseHTTPRequestHandler):
    ...
    @classmethod
    def reportPage(cls):
        cls.quiz_item_correct_answer = cls.quiz_item.correct_answer
        template_substitutions = {
            'number': cls.quiz_count,
            'score': cls.quiz_score,
            'length': len(cls.one_quiz),
            'question': cls.quiz_item.question,
            'answers': cls.quiz_item_answers,
            'hidden': [""] * len(cls.quiz_item),
            'answer': "",
            'report': f"score: {cls.quiz_score} question {cls.quiz_count} / {len(cls.one_quiz._items)}"
        }
    ...
    def do_GET(self):
        ...
        if self.path == "/report":
            ...
            self.send_header("Content-type", "text/html")
            self.end_headers()
            self.wfile.write(bytes(self.reportPage(), "utf-8"))
